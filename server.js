...
let blobContentFilterRules = []; // --- THÊM MỚI ---: Mảng lưu các rule để lọc NỘI DUNG của blob
...
const updateDetectionRules = async () => {
    ...
    try {
        ...
        blobRulesRaw.forEach(r => {
            try {
                // Lấy phần regex từ 'regex:blob:...' để lọc nội dung
                blobContentFilterRules.push(new RegExp(r.substring(11).trim(), 'i'));
            } catch (e) { console.error(`[RULE MANAGER] Lỗi cú pháp rule lọc blob: "${r}". Bỏ qua.`); }
        });

        console.log(`[RULE MANAGER] Cập nhật thành công! ${networkDetectionRules.length} rule mạng, ${blobContentFilterRules.length} rule lọc nội dung blob.`);
        if (blobContentFilterRules.length > 0) {
            console.log(`[RULE MANAGER] Các rule lọc nội dung blob: ${blobContentFilterRules.map(r => r.source).join(', ')}`);
        }
...
async function handleScrapeRequest(targetUrl, headers) {
    ...
    try {
        ...
        // --- GIAI ĐOẠN 3: Bắt và LỌC BLOB (phương án cuối cùng) ---
        ...
        if (interceptedBlobUrls.size > 0) {
            for (const blobUrl of interceptedBlobUrls) {
                ...
                if (blobContent) {
                    // --- LỌC NỘI DUNG BLOB ---
                    const isContentMatch = blobContentFilterRules.some(rule => rule.test(blobContent));
                    if (isContentMatch) {
                        console.log(`[BLOB FILTER] Nội dung từ ${blobUrl} khớp với rule. Đã lưu lại.`);
                        foundContents.add(blobContent);
                    } else {
                        console.log(`[BLOB FILTER] Nội dung từ ${blobUrl} không khớp với rule. Bỏ qua.`);
                    }
                }
            }
        }
...
regex:\\.m3u8(\\?|$)</code></pre><h3>Rule Lọc Nội Dung Blob (Mới)</h3><p>Sử dụng tiền tố <code>regex:blob:</code> để lọc nội dung text bên trong blob. Tính năng bắt blob chỉ được kích hoạt nếu có ít nhất một rule loại này.</p><pre><code># Chỉ giữ lại các blob M3U8 có chứa link trỏ đến domain 'kjl.bit'
regex:blob:kjl\\.bit

# Chỉ giữ lại các blob có chứa chuỗi 'level=4'
regex:blob:level=4</code></pre></div><h2><span class="badge badge-get">GET</span> /api/scrape</h2><div class="endpoint"><h3>Ví dụ</h3><pre><code>curl "http://localhost:3000/api/scrape?url=...&key=..."</code></pre></div></body></html>`;
